...
def smLogit_importance(train_x, train_y,feats,label='y', classifier_regressor=1,save_file="./smlogit_importance.csv"):
    print('x:',train_x[:5])
    print('y:',train_y[:5])
    clf = sm.OLS(train_y , train_x)  if classifier_regressor==1 else sm.Logit(train_y , train_x) 
    clf = clf.fit()    
...
def data_anlalyse(df,label ,pic_path='./feature_select/'):
    ...
    categorical_var, numerical_var = dp.category_continue_separation( df, feature_names )
    ...
    # 分箱看异常值情况
    categorical_var, numerical_var = dp.category_continue_separation( df,feature_names=df.columns.tolist() )
...
if  __name__ == '__main__':
    ...
    importance_desc = smLogit_importance( iris.data, iris.target, feats=iris.feature_names, classifier_regressor=1,
