...
def cal_advantage(temp, piont, method,flag='sel'):
    """计算当前切分点下的指标值
    ##参数  temp: 上一步的分箱结果，pandas dataframe
    piont: 切分点，以此来划分分箱  ;   method: 分箱方法选择，1:chi-merge , 2:IV值, 3:信息熵"""
    #temp = binDS
    if flag == 'sel':
...
def best_split(df_temp0, method, bin_num):
    ...
    ##返回值      返回在本次分箱标号内的最有切分结果， pandas dataframe """
#   df_temp0 = df_temp
#   bin_num = 1
...
def select_split_point(temp_bin, method):
    """二叉树分割方式，从候选者中挑选每次的最优切分点，与切分后的指标计算，是cont_var_bin函数的中间过程函数，
    ##参数   temp_bin: 分箱后的结果 pandas dataframe;    method:分箱方法选择，1:chi-merge , 2:IV值, 3:信息熵
    ##返回值 新的分箱结果类型是pandas dataframe"""
    #temp_bin = df_temp_all
    temp_bin = temp_bin.sort_values(by=['bin', 'bad_rate'])
    ##得到最大的分箱值
    max_num = max(temp_bin['bin'])                   #select_split_point 之前已经降bin都置为1
#    temp_binC = dict()
#    m = dict()
#    ##不同箱内的数据取出来
...
#        m[i] = len(temp_binC[i])
    ...
    bin_i_value = []
    for i in range(1, max_num + 1):
        df_temp = temp_bin[ temp_bin['bin'] == i ]  #select_split_point 之前已经降bin都置为1
        if df_temp.shape[0]>1:    #
            ...
            temp_split['bin'] = np.where(temp_split['split'] == 1,
                                               max_num + 1,
                                               temp_split['bin'] )   #这里是为啥要换成    max_num+1
            ##取出bin!=i合并为新租
...
def limit_min_sample(temp_cont,  bin_min_num_0):
    """分箱约束条件：每个箱内的样本数不能小于bin_min_num_0，cont_var_bin函数需要的中间过程函数
    temp_cont:      初始化分箱后的结果 pandas dataframe
    bin_min_num_0:  每组内的最小样本限制
    ##返回值：       合并后的分箱结果，pandas dataframe
    """
...
def cont_var_bin(x, y, method, mmin=5, mmax=10, bin_rate=0.01, stop_limit=0.1, bin_min_num=20):
    ...
    bin_init    = init_equal_bin(x, bin_rate)                                       # 得到等分箱,这里没有对NA分箱
    ##分箱映射
    bin_map     = cont_var_bin_map(x, bin_init)                                     #将x 按分箱转化，x的名字加"__BIN"
    df_temp     = pd.concat([x, y, bin_map], axis=1 )                               #  bin_map 是做了区分后的
    df_temp_1   = pd.crosstab(index=df_temp[bin_map.name], columns=y  )             # 做统计看各分段的好坏频数，index是行columns是列
    df_temp_1.rename(columns= dict(zip([0,1],['good', 'bad']) ),inplace=True)       # 0:'good', 1:'bad'

    df_temp_2   = pd.DataFrame( df_temp.groupby(bin_map.name).count().iloc[:, 0] )  # 计算每个bin中一共有多少样本
    df_temp_2.columns = ['total']
    df_temp_all = pd.merge( pd.concat([df_temp_1, df_temp_2], axis=1), bin_init,    # df_temp_all 是汇总
    ...
    if mmin >= df_temp_all.shape[0]:  # 如果小于最低要求
        gain_value_save0    =0
        gain_rate_save0     =0
...
def disc_var_bin(x, y, method=1, mmin=3, mmax=8, stop_limit=0.1, bin_min_num = 20  ):
    ...
    ##参数：     x:输入分箱数据，pandas series ;    y:标签变量
    method:分箱方法选择，1:chi-merge , 2:IV值, 3:信息熵
    mmin:最小分箱数，当分箱初始化后如果初始化箱数小于等mmin，则mmin=2，即最少分2箱，
...
import pickle
def save_var_bin(var_bin_base, filename):
    save_model = open( filename , 'wb')
    pickle.dump(var_bin_base, save_model, 0)
    save_model.close()
    return

def load_var_bin(filename):
    read_model = open( filename , 'rb')
    var_bin = pickle.load(read_model)
    read_model.close()
    return var_bin
...
#    ss = data_test[list( dict_disc_bin.keys())]
    ...
    save_var_bin(  dict_disc_bin[i], 'variable_bin_methods.pkl'  )
    var_bin     =   load_var_bin(  './variable_bin_methods.pkl' )
    print( dict_disc_bin.keys()          )
    print( dict_disc_bin['present_emp']  )
