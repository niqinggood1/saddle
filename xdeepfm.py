...
class Dense_layer(Layer):
    def __init__(self, hidden_units, out_dim=1, activation='relu', dropout=0.3):
...
class xDeepFM(Model):
    def __init__(self, dense_last_idx,sparse_fea_dim, cin_size,
                 dense_hidden_units, dense_output_dim=1, dense_hidden_activation='relu', dropout=0.2):
        ...
        self.embed_layers = [Embedding(feat['feat_onehot_dim'], feat['embed_dim'])
                             for feat in self.sparse_fea_dim]
        self.linear = Linear()
        self.dense_layer = Dense_layer( dense_hidden_units, dense_output_dim, dense_hidden_activation, dropout )
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        emb = tf.transpose(tf.convert_to_tensor(emb), [1, 0, 2]) # [None, n, k] perfect
...
class exDeepFM(Model):
    def __init__(self, feature_columns,sparse_fea_dim, cin_size, hidden_units, out_dim=1, activation='relu', dropout=0.0):
        super(xDeepFM, self).__init__()
